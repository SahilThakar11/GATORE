...
interface GatoreLogoProps {
  className?: string;
  animateDie?: boolean;
}

export function GatoreLogo({ className, animateDie = false }: GatoreLogoProps) {
  const [animDuration, setAnimDuration] = useState("1s");

  useEffect(() => {
    if (!animateDie) return;
    const from = 1;
    const to = 5;
    const over = 3000;
    ...
    const delay = setTimeout(() => {
      ...
      interval = setInterval(() => {
        ...
        if (elapsed >= over) {
          setAnimDuration(`${to}s`);
          clearInterval(interval);
          return;
        }
        const t = elapsed / over;
        const eased = 1 - Math.pow(1 - t, 3); // cubic ease-out
        setAnimDuration(`${(from + (to - from) * eased).toFixed(2)}s`);
      }, 50);
    }, 3000);
  ...
  return (
    <svg
      ...
      className={className ?? "w-16 h-8 sm:w-[93px] sm:h-12"}
      fill="none"
    ...
    >
      ...
      {/* Die */}
      <g
        className={animateDie ? "animate-spin" : ""}
        style={
          animateDie
            ? { transformBox: "fill-box", transformOrigin: "center", animationDuration: animDuration }
            : undefined
        }
      >
        <path
          fill="#096654"
          d="M271.9,143.8l-16.48-53.33c-.5-3.1.23-4.63,2.11-6.95,3.65-4.5,10.64-10.57,15.09-14.69,6.07-5.62,12.4-11.39,18.73-16.68,7.06-5.91,13.28-2.14,21.38-.88,10.48,1.63,25.46,2.37,35.15,5.49,2.6.84,2.84,2.8,3.72,5.13,2.18,5.81,4.16,13.08,5.81,19.13,2.22,8.14,5.63,19.65,6.86,27.74.13.87.22,1.84.08,2.7-.31,1.9-9.38,12.91-11.26,15.29-5.88,7.47-14.47,19.34-21.09,25.58-.54.51-2.2,1.86-2.84,1.99-2.03.39-6.17-.19-8.39-.45-16.21-1.88-32.9-6.69-48.85-10.09ZM339.41,56.17l-40.63-5.04c1.25,1.59,3.7,8.31,5.43,8.65l35.19-3.61ZM295.76,51.48c-.9,0-4.99,3.46-6.03,4.29-8.78,7.05-18.08,15.82-26.17,23.72-.97.95-5.17,4.79-5.02,5.84.11.78.55.44,1.01.22.64-.31,1.41-.79,2.08-1.15,13.13-7.13,25.57-15.73,38.76-22.8.91-.98-.28-3.35-.74-4.5-.52-1.31-2.43-5.61-3.89-5.62ZM352.29,102.43c-.1-3.68.14-7.4.02-11.08-.23-7.21-.78-15.76-1.63-22.91-.43-3.63-.28-8.9-4.41-10.08-4.65-1.33-21.44.75-27.17,1.43-4.78.56-9.61,1.34-14.29,2.42,9.49,8.8,19.27,17.43,29.18,25.74,5.95,4.99,11.91,10.12,18.29,14.49ZM300.79,63.81l-24.26,13.36c-2.34,1.63-18.14,10.4-18.29,12.19,9.19,11.74,18.68,23.32,29.08,34l13.47-59.54ZM345.7,100.17c-9.3-8.89-20.58-18.19-30.64-26.3-2.49-2.01-7.91-6.88-10.42-8.09-.49-.23-.82-.51-1.43-.37-5.35,19.49-10.36,39.18-13.68,59.14l60.35-19.72c-1.01-1.86-2.66-3.22-4.17-4.67ZM353.89,73.06c-1.48-.34-.4,1.65-.38,2.6.19,9.1.18,18.2.47,27.3.46,1.52,7.18,5.91,7.96,5.11-1.56-11.92-4.75-23.5-8.05-35ZM278.41,135.98c1.58-1.61,6.91-7.18,7.33-9,.07-.28.16-.51-.03-.78-7.17-9.03-13.98-18.46-21.55-27.17-1.37-1.58-3.06-3.76-4.62-5.04-.45-.37-.65-.98-1.41-.8l14.89,47.47c2.19-1.08,3.74-3.01,5.38-4.68ZM351.48,106.46c-17.14,5.36-34.54,10.31-51.48,16.31-2.6.92-11.67,2.9-8.03,6.39s10.96,7.89,15.29,10.86c2.03,1.39,18.69,12.95,19.63,12.28,5.82-8.85,11.37-17.95,16.35-27.32,1.17-2.2,9.24-17.46,8.24-18.51ZM330.16,151.52c.76.17.95-.43,1.41-.8,6.64-5.47,15.4-19.44,21.14-26.74,1.98-2.53,8.81-9.48,9.48-11.87.29-1.04-2.32-2.45-3.19-2.9-.93-.48-4.24-1.98-5.04-1.48l-23.79,43.79ZM321.71,152.72c-7.63-5.98-15.55-11.81-23.61-17.22-1.76-1.18-8.99-6.45-10.57-5.91-2.83.97-9.49,10.01-12.49,12.08-.18.92.4.5.95.65,15.07,4.08,30.34,7.71,45.72,10.41Z"
        />
        <path
          fill="#f0fdfa"
          d="M345.7,100.17c1.51,1.44,3.16,2.81,4.17,4.67l-60.35,19.72c3.32-19.96,8.33-39.65,13.68-59.14.61-.14.95.14,1.43.37,2.51,1.2,7.93,6.08,10.42,8.09,10.06,8.11,21.34,17.41,30.64,26.3Z"
        />
        <path
          fill="#ffffff"
          d="M320.01,86.03c-8.63,1.25-4.89,18.29,2.41,19.53,12.25,2.08,7.69-20.99-2.41-19.53ZM308.04,108.07c1.38-4.63,6.82-12.87.68-16.18-5.16-2.78-10.89,1.51-9.53,7.32,3.69.15,2.3-2.2,3.44-4,1.5-2.37,6.06-1.53,6.16,1.77.08,2.71-6.16,14.92-5.58,15.5l12.48-4.25c-.46-4.3-5.07-.68-7.65-.17Z"
        />
        <path
          fill="#096654"
          d="M320.01,86.03c10.1-1.46,14.66,21.61,2.41,19.53-7.3-1.24-11.03-18.28-2.41-19.53ZM319.99,88.83c-4.08,1.21-1.28,13.33,3.16,13.99,7.47,1.1,2.72-15.74-3.16-13.99Z"
        />
        <path
          fill="#096654"
          d="M308.04,108.07c2.58-.51,7.19-4.13,7.65.17l-12.48,4.25c-.58-.58,5.66-12.79,5.58-15.5-.1-3.3-4.66-4.14-6.16-1.77-1.14,1.8.25,4.15-3.44,4-1.36-5.82,4.37-10.1,9.53-7.32,6.14,3.3.7,11.54-.68,16.18Z"
        />
        <path
          fill="#ffffff"
          d="M319.99,88.83c5.88-1.75,10.63,15.09,3.16,13.99-4.44-.66-7.24-12.77-3.16-13.99Z"
        />
        <path
          fill="#ffffff"
          d="M351.48,106.46c1.01,1.05-7.07,16.32-8.24,18.51-4.97,9.37-10.53,18.47-16.35,27.32-.95.67-17.6-10.88-19.63-12.28-4.33-2.97-11.74-7.45-15.29-10.86s5.43-5.47,8.03-6.39c16.94-6,34.34-10.94,51.48-16.31Z"
        />
        <path
          fill="#ffffff"
          d="M300.79,63.81l-13.47,59.54c-10.39-10.68-19.89-22.26-29.08-34,.15-1.78,15.95-10.55,18.29-12.19l24.26-13.36Z"
        />
        <path
          fill="#ffffff"
          d="M352.29,102.43c-6.38-4.38-12.35-9.51-18.29-14.49-9.91-8.31-19.69-16.94-29.18-25.74,4.68-1.07,9.5-1.85,14.29-2.42,5.73-.67,22.52-2.75,27.17-1.43,4.12,1.18,3.98,6.45,4.41,10.08.85,7.15,1.4,15.71,1.63,22.91.12,3.68-.12,7.4-.02,11.08Z"
        />
        <path
          fill="#ffffff"
          d="M278.41,135.98c-1.63,1.67-3.18,3.6-5.38,4.68l-14.89-47.47c.76-.18.95.43,1.41.8,1.55,1.28,3.25,3.47,4.62,5.04,7.57,8.71,14.37,18.14,21.55,27.17.19.27.1.5.03.78-.43,1.82-5.75,7.39-7.33,9Z"
        />
        <path
          fill="#ffffff"
          d="M321.71,152.72c-15.38-2.7-30.65-6.32-45.72-10.41-.54-.15-1.13.28-.95-.65,3.01-2.07,9.66-11.1,12.49-12.08,1.58-.54,8.8,4.73,10.57,5.91,8.06,5.41,15.98,11.25,23.61,17.22Z"
        />
        <path
          fill="#ffffff"
          d="M295.76,51.48c1.46,0,3.37,4.3,3.89,5.62.46,1.15,1.65,3.52.74,4.5-13.19,7.06-25.63,15.67-38.76,22.8-.66.36-1.44.84-2.08,1.15-.46.22-.9.56-1.01-.22-.15-1.05,4.05-4.89,5.02-5.84,8.09-7.89,17.39-16.66,26.17-23.72,1.03-.83,5.13-4.3,6.03-4.29Z"
        />
        <path
          fill="#ffffff"
          d="M330.16,151.52l23.79-43.79c.8-.5,4.11,1,5.04,1.48.88.45,3.49,1.86,3.19,2.9-.68,2.39-7.5,9.35-9.48,11.87-5.74,7.3-14.5,21.27-21.14,26.74-.45.37-.65.98-1.41.8Z"
        />
        <path
          fill="#ffffff"
          d="M339.41,56.17l-35.19,3.61c-1.74-.34-4.18-7.06-5.43-8.65l40.63,5.04Z"
        />
        <path
          fill="#ffffff"
          d="M353.89,73.06c3.3,11.5,6.49,23.08,8.05,35-.79.8-7.5-3.59-7.96-5.11-.29-9.1-.28-18.2-.47-27.3-.02-.95-1.11-2.93.38-2.6Z"
        />
      </g>
    </svg>
  );
