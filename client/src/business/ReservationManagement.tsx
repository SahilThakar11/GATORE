...
const MockReservations = [
    {
        ...
        phone: "555‑123‑4567",
        notes: "Birthday celebration – please prepare table with decorations if available"
    },
    {
        id: 2,
...
const TimelineView = () => {
    ...
    const tables = Array.from(new Set(timelineReservations.map(r => r.table))).sort();
        ...
        <div className="bg-white border border-warm-200 rounded-xl shadow p-6 mb-6 overflow-x-auto">
            <div className="flex items-center justify-between mb-6">
                <div>
                    ...
                    <p className="text-sm text-gray-500">Visualize reservation timing and table overlaps</p>
                </div>
            ...
            <div className="min-w-max">
                ...
                <div className="divide-y divide-gray-300">
                    {tables.map((table) => {
                        const tableReservations = timelineReservations.filter(r => r.table === table);

                        return (
                            <div key={table} className="flex bg-gray-50 hover:bg-gray-100 transition">
                                {/* Table Label */}
                                ...
                                <div className="flex relative">
                                    ...
                                    {tableReservations.map((res) => {
                                        ...
                                        return (
                                            ...
                                            >
                                                <div className={`${getStatusColor(res.status)} text-white rounded-lg px-3 py-1 h-full flex flex-col justify-between`}>
                                                    <div>
                                                        <div className="flex items-center justify-between gap-1">
                                                            <p className="font-semibold text-xs">{res.customerName}</p>
                                                            {res.badge && (
                                                                <span className={`${getBadgeColor(res.badge)} text-xs px-1.5 py-0.5 rounded font-semibold whitespace-nowrap`}>
                                                                    {res.badge}
                                                                </span>
                                                            )}
                                                        </div>
                                                        <div className="flex gap-3">
                                                            <p className="text-xs text-gray-100 mt-0.5">🧑‍🤝‍🧑 {res.partySize}</p>
                                                            <p className="text-xs text-gray-100">🕐 {Math.floor(res.startTime)}:{String((res.startTime % 1) * 60).padStart(2, "0")} PM</p>
                                                        </div>
    ...
    return (
			<div className="bg-white border border-warm-200 rounded-xl shadow p-6 mb-6">
                <div className="flex items-center justify-between mb-4">
                    <div>
                        <h3 className="text-xl font-semibold text-gray-800">Floor Plan View</h3>
        ...
        <div className="bg-teal-100 border border-teal-600 rounded-2xl shadow p-6">
                    ...
                    <div className="flex items-center space-x-2">
                        ...
                        <button className="text-gray-600 hover:text-gray-800 flex items-center">
                            <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                                ...
                                <path fillRule="evenodd" d="M5 14a5 5 0 0110 0v1H5v-1z" clipRule="evenodd" />
                            </svg>
                            <span className="text-xs ml-1">{reservation.partySize}</span>
                        </button>
                    </div>
                    <div className="flex items-center space-x-2">
                        <button className="bg-white border border-gray-300 text-gray-700 text-sm px-3 py-1 rounded-full">{reservation.status}</button>
                        <button className="bg-teal-600 hover:bg-emerald-600 text-white text-sm px-3 py-1 rounded">Complete</button>
                        <button className="text-gray-600 hover:text-gray-800" onClick={toggleCollapse}>
                            <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                                <path d="M2 5a2 2 0 012-2h12a2 2 0 012 2v8a2 2 0 01-2 2H6l-4 4V5z" />
                            </svg>
                        </button>
            ...
            <div className={`transition-max-height duration-300 ease-in-out overflow-hidden ${isOpen ? 'max-h-screen' : 'max-h-0'}`}>
                ...
                <div className="grid grid-cols-2 gap-6">
                    <div>
                        <p className="text-sm font-semibold text-gray-600 mb-2">Reservation Details</p>
                        <div className="space-y-1 text-sm text-gray-700">
                            <p><span className="inline-block w-4">📅</span> {reservation.date}</p>
                            <p><span className="inline-block w-4">⏰</span> {reservation.time} (2 hours)</p>
                            <p><span className="inline-block w-4">🎲</span> Wingspan</p>
                        </div>
                        <div className="mt-4">
                            <p className="text-sm font-semibold text-gray-600 mb-2">Customer History</p>
                            <p className="text-sm text-gray-700">⏱️ 7 previous visits</p>
                        </div>
                    </div>
                    <div>
                        <p className="text-sm font-semibold text-gray-600 mb-2">Contact Information</p>
                        <div className="space-y-1 text-sm text-gray-700">
                            <p>✉️ {reservation.email}</p>
                            <p>📞 {reservation.phone}</p>
                        </div>
                        <div className="mt-4">
                            <p className="text-sm font-semibold text-gray-600 mb-2">Booking Info</p>
                            <p className="text-sm text-gray-700">ID: {reservation.id}</p>
                            <p className="text-sm text-gray-700">Type: <span className="text-blue-500">{reservation.reservationType}</span></p>
                            <p className="text-sm text-gray-700">Booked: {reservation.bookingTime}</p>
                ...
                <div className="flex justify-between items-center mt-6">
                    <div className="space-x-2">
                        <button className="border border-teal-600 text-teal-600 text-sm px-4 py-2 rounded hover:bg-teal-100 bg-white">Modify</button>
                        <button className="border border-teal-600 text-teal-600 text-sm px-4 py-2 rounded hover:bg-teal-100 bg-white">Send Reminder</button>
                    </div>
                    <div className="flex items-center space-x-2">
                        <button className="border border-red-500 text-red-500 text-sm px-4 py-2 rounded hover:bg-red-50 bg-white">Cancel</button>
